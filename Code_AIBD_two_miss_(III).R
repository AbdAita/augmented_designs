mu.ARIBD = function(obs, block, treatment,type, y,n_block_i,n_block_ii,n_check_j,n_check_jj) 
{
  ...
  nn_new=t(x1)%*%x4[,2]
  n_new=nn_new[1]
  mu_q=((b*c-b-c)*sumcheck+c*(yBM..[j]+yBM..[jj])+b*(yBi..[i]+yBi..[ii]))/(b*c*(b*c-b-c+2))
  beta=c()
  for (a in 1:b) { 
    if(a==i){
      beta[a]<-(b-1)*((b-1)*(c-1)*(yBi..[i]-yBM..[jj]/(b-1))-(yBi..[ii]-yBM..[j]/(b-1))-((c*b-c-b)*(sum(yBM..)-yBM..[j]-yBM..[jj]))/b)/((b*c-c-b)*((c-1)*(b-1)+1))
    }
    if(a==ii){
      beta[a]<-(b-1)*((b-1)*(c-1)*(yBi..[ii]-yBM..[j]/(b-1))-(yBi..[i]-yBM..[jj]/(b-1))-((c*b-c-b)*(sum(yBM..)-yBM..[j]-yBM..[jj]))/b)/((b*c-c-b)*((c-1)*(b-1)+1))
    }
    if(a!=i & a!=ii){
      beta[a]<-(yBi..[a])/c-mu_q
    }
  }
  mu=(y....-((b-1)*(sum(yBM..)-yBM..[j]-yBM..[jj])/b)-((b-2)*(yBM..[j]+yBM..[jj])/(b-1))-sum(nn_new*beta)+nn_new[i]*beta[i]+nn_new[ii]*beta[ii]+((1-(b-1)*nn_new[i])*beta[i])/(b-1)+((1-(b-1)*nn_new[ii])*beta[ii])/(b-1))/(n_t+c)
...
Beta.ARIBD = function(obs, block, treatment,type, y,n_block_i,n_block_ii,n_check_j,n_check_jj) 
{
  ...
  for (a in 1:b) { 
    if(a==i){
      beta_RT[a]<-(yB...[i])/(c+nn_new[i]-1)-mu_RT
    }
    if(a==ii){
      beta_RT[a]<-(yB...[ii])/(c+nn_new[ii]-1)-mu_RT
    }
    if(a!=i & a!=ii){
...
Check.ARIBD = function(obs, block, treatment,type, y,n_block_i,n_block_ii,n_check_j,n_check_jj) 
{
  ...
  for (e in 1:c) { 
    ...
    if(e==jj){
      taucheck[e]<-(yBM..[jj]+beta[ii])/(b-1)-mu
    }
    if(e!=j & e!=jj){
...
aov1.ARIBD = function(obs, block, treatment,type, y,n_block_i,n_block_ii,n_check_j,n_check_jj)
{
  ...
  for (e in 1:c) { 
    ...
    if(e==jj){
      tauCheck[e]<-(yBM..[jj]+beta[ii])/(b-1)-mu_q
    }
    if(e!=j & e!=jj){
...
########################################
######################################## 
TMC.III.ARIBD = function(obs, block, treatment,type, y,n_block_i,n_block_ii,n_check_j,n_check_jj)
{
